...
- **start** registers this node once — `.agent` in the agent directory is its
  ...
  the platform's own service. The PAT is passed as `VSTS_AGENT_INPUT_TOKEN`
  rather than `--token`, so it does not show up in the process list of the jobs
  the agent later spawns.
