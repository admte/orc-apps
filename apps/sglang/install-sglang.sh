...
# Installs one SGLang release into its own virtual environment and creates the
...
ensure_venv() {
	...
	echo "sglang install: installing the python3 virtual environment package" >&2
...
*) fail "SGLang publishes x86-64 and aarch64 wheels; this node is $(uname -m)" ;;
esac
...
if ! command -v nvidia-smi >/dev/null 2>&1; then
	note "warning: nvidia-smi is not on this node; SGLang will not start without an NVIDIA GPU"
fi
...
# and nothing newer.
#
# The plain package, not `sglang[all]`: from 0.5 on, the serving stack — torch,
# flashinfer, sgl-kernel — is in the base dependencies, and `all` adds only the
# diffusion, http2 and tracing extras, gigabytes of image and video code an
# inference server never runs. On releases before 0.5 the stack lived in the
# `srt` extra, which is why the torch check below exists.
note "installing sglang==$APP_VERSION — the wheel set and its CUDA payload are gigabytes"
"$venv/bin/pip" install --no-cache-dir --upgrade pip >/dev/null ||
	fail "could not update pip in $venv"
"$venv/bin/pip" install --no-cache-dir "sglang==$APP_VERSION" ||
	fail "could not install sglang==$APP_VERSION"

# SGLang ships no console script: the server is a module. Checked from pip's own
# file list rather than by importing it — importing pulls in the whole serving
# stack, which an install node without a GPU has no reason to load.
"$venv/bin/pip" show -f sglang 2>/dev/null | grep -q 'sglang/launch_server\.py' ||
	fail "the wheel installed no sglang/launch_server.py in $venv"

# The base package carries the serving stack only from 0.5 on. Saying so here is
# what keeps an older release from installing cleanly and failing at first start.
"$venv/bin/pip" show torch >/dev/null 2>&1 ||
	fail "sglang $APP_VERSION did not bring torch with it; this package supports the releases that carry the serving stack in the base wheel — choose 0.5.0 or newer"

installed=$("$venv/bin/pip" show sglang 2>/dev/null | sed -n 's/^Version: //p')
...
note "sglang $installed installed at $prefix"
